...
process.stdin.on("data", function (data) {
  ...
  if (verifyExistence(route) && isFile(route)) {
    ...
    process.stdout.write(
      chalk.bgRedBright(
        chalk.black(
          "El archivo o directorio no existe. Verifique y vuelva a iniciar la librería"
        )
      )
    );
    exit();
  ...
  if (isFile(ruta) && verifyExtension(ruta)) {
    ...
    const linksMd = (files, options = { validate: false }) => {
      return new Promise((resolve, reject) => {
        ...
        console.log(chalk.red("Este es el " + err));
      });
    };
    ...
    process.stdout.write(
      chalk.bgRedBright(chalk.black("El archivo no es MD!"))
    );
    exit();
  }
});
